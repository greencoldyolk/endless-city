...
2. 紧跟**地图组标签**（`city_edge` / `city_interior` / `amusement_park`）和
   **色调标签**（`cool rainy blue hour` / `romantic purple twilight` / 夜景用
   `cold blue night`），文件名前缀 A=city_edge、B=city_interior、C=amusement_park
